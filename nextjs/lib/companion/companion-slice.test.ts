...
describe("mini app store + maximize", () => {
  ...
  it("ships store + phone control center wiring", () => {
    ...
    assert.match(apps, /data-cc-home/);
    assert.match(apps, /data-cc-store/);
    assert.match(apps, /data-cc-app=\{app\.id\}/);
    assert.match(apps, />\s*\+\s*</);
    assert.match(apps, /agregá apps/);
    assert.match(apps, /openFromSheet/);
    assert.match(apps, /goHome/);
    assert.match(apps, /loadInstalledApps/);
    assert.match(css, /@media \(max-width: 699px\)[\s\S]*\.app-dock-nav[\s\S]*display:\s*none/);
    assert.match(css, /\.app-dock \[data-phone-center\][\s\S]*display:\s*none/);
    assert.match(css, /@media \(max-width: 699px\)[\s\S]*\[data-phone-center\][\s\S]*64px/);
    assert.match(css, /\.phone-cc-add/);
    assert.doesNotMatch(apps, /hasOpen \? "apps" : "casa"/);
    assert.doesNotMatch(apps, /onToggleSwitcher/);
